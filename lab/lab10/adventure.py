...
def adv_parse(line):
    ...
    if tokens[0] == 'give':
        "*** YOUR CODE HERE ***"
        return (tokens[0], ' '.join(tokens[1:-2]) , tokens[-1])

    elif tokens[0] == 'ask':
        "*** YOUR CODE HERE ***"
        return (tokens[0], tokens[1], ' '.join(tokens[3:]))
...
def adv_eval(exp):
    ...
    elif exp[0] == 'give': # give something to somone
        "*** YOUR CODE HERE ***"
        return give(exp[2],Person(exp[1])) #(person,thing)

    elif exp[0] == 'ask':  #ask someone for something
        "*** YOUR CODE HERE ***"
...
class Place(object):
    ...
    def describe(self):
        raise TypeError(str(exp) + ' is not a string or call expression')
